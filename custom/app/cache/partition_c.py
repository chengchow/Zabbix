...
"""
说明: 
    该脚本用于创建分区缓存数据, 仅在CacheAuto设置为True时自动执行
...
def main():
    ## 获取分区列表
    partList = osCfg.partList

    ## 获取数据列表
    dataList = [ psutil_to_dict(psutil.disk_usage(x)) for x in partList ]

    ## 获取数据字典
    dataDict = dict(zip(partList, dataList))
